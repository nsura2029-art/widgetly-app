...
| `ADMIN_SESSION_SECRET`            | Generated locally (e.g. `openssl rand -hex 64`)                                                       | HMAC signing of the `wly_admin` session cookie (`src/lib/admin/auth.ts`). **Required on both stage and prod** — without it every admin login returns 500. Deploy workflow (`deploy.yml`) syncs it to the production Worker automatically. Stage requires a one-time manual `wrangler secret put ADMIN_SESSION_SECRET --env stage` on first stage deploy. | On suspected leak. Rotating invalidates every existing admin session (intended). |
